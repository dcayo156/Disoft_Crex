...
#Region "Eventos"
    ...
    Private Function P_fnGenerarFactura(numi As String, subtotal As Double, descuento As Double, total As Double, nit As String, Nombre As String, Codcli As String) As Boolean
        ...
        res = P_fnGrabarFacturarTFV001(numi, subtotal, descuento, total, nit, Nombre, Codcli, facturaId) ' Grabar en la TFV001
        ...
        If (res) Then
            ...
            If (P_fnValidarFactura()) Then
                ...
                P_prImprimirFacturar(numi, True, True, nit, facturaId) '_Codigo de a tabla TV001
            Else
    ...
    Private Sub P_prImprimirFacturar(numi As String, impFactura As Boolean, grabarPDF As Boolean, nit As String, facturaId As Integer)
        FacturaPrint.P_prImprimirFacturar(numi, impFactura, grabarPDF, nit, QrFactura, Me, facturaId, False)
    End Sub
    Private Sub P_ReImprImprimirFacturar(numi As String, impFactura As Boolean, grabarPDF As Boolean, nit As String, facturaId As Integer)
        FacturaPrint.P_prImprimirFacturar(numi, impFactura, grabarPDF, nit, QrFactura, Me, facturaId, True)
    End Sub
    ...
    Public Function P_fnImageToByteArray(ByVal imageIn As Image) As Byte()
        ...
        Return ms.ToArray()
    End Function
    Private Function P_fnGrabarFacturarTFV001(numi As String, subtotal As Double, descuento As Double, total As Double, nit As String, nameCliente As String, Codcli As String, ByRef facturaId As Integer) As Boolean
...
#End Region
    ...
    Private Sub btnFactura_Click(sender As Object, e As EventArgs) Handles btnFactura.Click
        Try
            ...
            For i As Integer = 0 To list2.Count - 1 Step 1
                ...
                Else
                    If (list2(i).NroFactura.Equals("") Or list2(i).NroFactura.Equals("0")) Then
                        list1.Add(list2(i))
                    Else
                        Dim totalFacturas As DataTable = L_ObtenerFacturas(list2(i).Id)
                        If totalFacturas.Rows.Count > 0 Then
                            For Each fila As DataRow In totalFacturas.Rows
                                P_ReImprImprimirFacturar(list2(i).Id, True, True, 0, fila("fvanumi"))
                            Next
                        End If
            ...
            CargarPedidos()
            MostrarMensajeOk("Facturas Generadas Correctamente")
        Catch ex As Exception
            MostrarMensajeError(ex.Message)
        End Try
    End Sub

    Private Sub IntroducirFactura(dtDetalle As DataTable)
